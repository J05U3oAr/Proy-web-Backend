...
Usamos Go por su rendimiento y simplicidad para construir servidores HTTP, además de que también esta fue la tecnología que se nos fue enseñada en clase. La librería estándar `net/http` es suficientemente poderosa sin necesidad de frameworks. `modernc.org/sqlite` fue una excelente elección porque compila sin CGO, sin embargo anteriormente se utilizó sql3 lo cuál generaba problema, por lo cual se tuvo que realizar un nuevo repositoriodebido a esos mismos problemas. Gracias a este cambio hizo que se simplificara mucho el proceso en Railway. Lo usaríamos de nuevo sin dudarlo, aunque para proyectos más grandes evaluaríamos PostgreSQL para aprovechar mejor la concurrencia.